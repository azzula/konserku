@section('title_page', ($identity === 'foryou' ? '- #ForYou' : ($identity === 'dashboard' ? '- Dashboard' : ($identity === 'tiket' ? '- Tiket' : ($identity === 'pengunjung' ? '- Pengunjung' : ($identity === 'laporan' ? '- Laporan' : '' ) ) ) ) ))

<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ $identity === 'foryou' ? '#ForYou' : ($identity === 'dashboard' ? 'Dashboard' : ($identity === 'tiket' ? 'Tiket' : ($identity === 'pengunjung' ? 'Pengunjung' : ($identity === 'laporan' ? 'Laporan' : '' ) ) ) ) }}
    </h2>
</x-slot>

<div class="pt-10 pb-10 px-2">
                        ...
                        <div class="relative">
                            <div class="flex items-center justify-between mb-1">
                                @include('livewire.includes.add')
                                    ...
                                    <!-- button create surat staff -->
                                    <div class="m-4 grid grid-cols-2">
                                        <div class="flex justify-end">
                                            <x-jet-button class="bg-green-500 hover:bg-green-700 my-auto" wire:click="confirmAdd()">{{ __('Buat') }}</x-jet-button>
                                        </div>
                                    </div>

                                <div class="flex items-center h-10 m-4">
                                    @if($this->searchableColumns()->count())
                                        <div class="flex rounded-lg w-full shadow-sm">
                                            <div class="relative flex-grow focus-within:z-10">
                                                ...
                                                <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Cari di')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
                                ...
                                @if($this->activeFilters)
                                    <span class="text-xl text-blue-400 uppercase">@lang('Filter aktif')</span>
                                @endif

                                <div class="flex flex-wrap items-center space-x-1 m-4">
                                    ...
                                    @if($exportable)
                                        <div x-data="{ init() {
                                            ...
                                            <button wire:click="export" class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('Ekspor') }}</span>
                                    ...
                                    <div class="items-center justify-between p-2 sm:flex">
                                        ...
                                        @if(count($this->results))
                                            ...
                                            <div class="flex justify-end text-gray-600">
                                                {{__('Hasil')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }} {{__('dari')}}
                                                {{ $this->results->total() }}
                                            </div>
                                        @endif
                                    </div>
